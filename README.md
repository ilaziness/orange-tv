...
一个开箱即用的视频管理系统，用来搭建面向观众的在线视频网站。支持内容管理、播放和后台运营，前端自适应手机、平板和电脑。

## 支持的平台

- web网页，自适应PC端，手机移动端，平板等各种屏幕大小
- 移动端APP，支持Android和IOS平台
- TV电视端，支持Android TV
...
- 在**首页**看到推荐视频、分类入口和最新内容
- 按**分类**（电影、电视剧、综艺、动漫等）浏览视频
- 按**地区、年份、语言**筛选和排序视频
- 进入**视频详情页**查看海报、简介、导演、演员、评分、播放源和剧集列表
...
| 部分                   | 使用者               | 用来做什么                                 |
| ---------------------- | -------------------- | ------------------------------------------ |
| **用户端**（前台网站） | 普通观众             | 浏览、搜索、观看视频                       |
| **管理后台**           | 站长 / 运营 / 录入员 | 管理视频内容、配置采集、管理用户、设置站点 |

## 管理后台能做的事

- **首页概况**：一眼看到今日新增视频数、总视频数、采集任务状态等运营数据
- **内容管理**
  - 分类管理：多级分类的增删改查、排序、启用/禁用
  - 视频管理：新增、编辑、删除、上下架、批量操作
...
如果你不想一部一部视频手动录入，可以使用采集功能自动拉取其他站点的视频数据：
...
创建成功后输出类似 `Created super_admin: id=1 username=admin`，即可用该账号登录管理端 `http://<host>:81`，开始录入或采集视频内容。

> 发布包内附带的 `README.md` 包含更详细的部署说明，可一并参考。

## 截图预览

> 以下截图仅用于功能演示，所展示的内容均为测试数据，不代表任何真实版权视频内容，如有侵权请联系删除。
...
4. 用命令行创建第一个管理员账号，然后登录管理后台开始录入或采集视频
...
本项目仅供学习交流和技术研究使用，不提供、不存储、不传播、不内置任何视频内容。

- 本系统源码与发布包内不内置任何视频内容、不内置任何播放数据，所有播放内容均为使用者自行录入
