...
type UserPlayHistory struct {
	...
	EpisodeID uint32 `bun:"episode_id,notnull" json:"episode_id"`
	// 播放进度（秒）
...
var _ bun.BeforeAppendModelHook = (*UserPlayHistory)(nil)

func (m *UserPlayHistory) BeforeAppendModel(ctx context.Context, query bun.Query) error {
	now := time.Now()
	switch query.(type) {
