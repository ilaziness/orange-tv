...
type fakePlayRepo struct {
	sources  map[int64]*model.PlaySources
	episodes map[int64]*model.PlayEpisodes
	nextID   int64
}
...
func (f *fakePlayRepo) GetEpisodeByKey(ctx context.Context, videoID, sourceID int64, episodeNumber int32) (*model.PlayEpisodes, error) {
	for _, ep := range f.episodes {
		if ep.VideoID == videoID && ep.SourceID == sourceID && ep.EpisodeNumber == episodeNumber {
			cp := *ep
			return &cp, nil
		}
	}
...
func (f *fakePlayRepo) ExistsEpisode(ctx context.Context, videoID, sourceID int64, episodeNumber int32, excludeID int64) (bool, error) {
	for id, ep := range f.episodes {
		if id == excludeID || ep.DeletedAt != nil {
			continue
		}
		if ep.VideoID == videoID && ep.SourceID == sourceID && ep.EpisodeNumber == episodeNumber {
			return true, nil
...
func (v *videoRepoStub) GetByID(ctx context.Context, id int64) (*model.Videos, error) {
	item, ok := v.videos[id]
	if !ok {
		return nil, nil
	}
...
func (v *videoRepoStub) SoftDelete(ctx context.Context, id int64) error        { return nil }
func (v *videoRepoStub) ReplaceDirectors(ctx context.Context, videoID int64, directorIDs []int64) error {
	return nil
}
func (v *videoRepoStub) ReplaceActors(ctx context.Context, videoID int64, actors []model.VideoActors) error {
	return nil
}
func (v *videoRepoStub) ReplaceTags(ctx context.Context, videoID int64, tagIDs []int64) error {
	return nil
}
func (v *videoRepoStub) ListDirectorIDs(ctx context.Context, videoID int64) ([]int64, error) {
	return nil, nil
}
func (v *videoRepoStub) ListActorRels(ctx context.Context, videoID int64) ([]model.VideoActors, error) {
	return nil, nil
}
func (v *videoRepoStub) ListTagIDs(ctx context.Context, videoID int64) ([]int64, error) {
	return nil, nil
}
