...
type fakeAdminRepo struct {
	admins map[uint32]*model.Admins
	groups map[uint32]*model.UserGroups
}
...
func (f *fakeAdminRepo) GetByID(ctx context.Context, id uint32) (*model.Admins, error) {
	a, ok := f.admins[id]
...
func (f *fakeAdminRepo) GetGroupByID(ctx context.Context, id uint32) (*model.UserGroups, error) {
	g, ok := f.groups[id]
...
func (f *fakeAdminRepo) Create(ctx context.Context, admin *model.Admins) error {
	admin.ID = uint32(len(f.admins) + 1)
	cp := *admin
	f.admins[admin.ID] = &cp
	return nil
}

func (f *fakeAdminRepo) UpdateLastLogin(ctx context.Context, id uint32, at time.Time) error {
...
func (f *fakeAdminRepo) UpdateProfile(ctx context.Context, adminID uint32, nickname, email, avatar string) error {
	if a, ok := f.admins[adminID]; ok {
		a.Nickname = nickname
		a.Email = email
		a.Avatar = avatar
	}
...
func (f *fakeAdminRepo) UpdatePassword(ctx context.Context, adminID uint32, hashedPassword string) error {
	if a, ok := f.admins[adminID]; ok {
...
func (f *fakeAdminRepo) SoftDeleteAdmin(ctx context.Context, id uint32) error       { return nil }
func (f *fakeAdminRepo) ExistsUsernameExcludeID(ctx context.Context, username string, excludeID uint32) (bool, error) {
	return false, nil
}
...
func (f *fakeAdminRepo) SoftDeleteGroup(ctx context.Context, id uint32) error       { return nil }
func (f *fakeAdminRepo) ExistsGroupNameExcludeID(ctx context.Context, name string, excludeID uint32) (bool, error) {
	return false, nil
}
