...
// Only client-visible groups (site/feature/seo) are exposed via the handler's binding tag.
type ClientSettingsService interface {
	...
	// Feature group is flattened for the request's client type.
...
func (s *clientSettingsService) GetByGroups(ctx context.Context, groups []string) (any, error) {
	...
	return s.shared.MapGroupsToResponse(groups, maps, clienttype.FromContext(ctx))
}
