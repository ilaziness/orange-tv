...
export const clientApi = {
  ...
  addFavorite: (videoId: number) =>
    withAuth((token) =>
      apiPost(CLIENT_API_BASE, `/favorites/${videoId}`, null, { token }),
    ),
  removeFavorite: (videoId: number) =>
    withAuth((token) =>
      apiDelete(CLIENT_API_BASE, `/favorites/${videoId}`, { token }),
    ),
  ...
  getHistory: (videoId: number) =>
    withAuth((token) =>
      apiGet<HistoryItem>(CLIENT_API_BASE, `/history/${videoId}`, { token }),
    ),
  ...
  deleteHistory: (videoId: number) =>
    withAuth((token) =>
      apiDelete(CLIENT_API_BASE, `/history/${videoId}`, { token }),
    ),
  clearHistory: () =>
    withAuth((token) => apiDelete(CLIENT_API_BASE, "/history", { token })),
