              ...
              <TableRow key={s.id}>
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    <Button size="sm" variant="ghost" onClick={() => onEdit(s)} disabled={scheduleBusy}>
                      <Pencil data-icon="inline-start" />
                      编辑
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => void onBindCategory(s.id)} disabled={scheduleBusy}>
                      <Network data-icon="inline-start" />
                      绑定分类
                    </Button>
                    {(s.schedule_enabled ?? 0) === 1 ? (
                      <Button size="sm" variant="ghost" onClick={() => void onDisableSchedule(s.id)} disabled={scheduleBusy}>
                        {scheduleBusy ? <Spinner data-icon="inline-start" /> : <Clock data-icon="inline-start" />}
                        取消定时
                      </Button>
                    ) : (
                      <Button size="sm" variant="ghost" onClick={() => void onEnableSchedule(s.id)} disabled={scheduleBusy}>
                        {scheduleBusy ? <Spinner data-icon="inline-start" /> : <Clock data-icon="inline-start" />}
                        启用定时
                      </Button>
                    )}
                    <Button size="sm" variant="ghost" onClick={() => void onCollectNow(s.id)} disabled={scheduleBusy}>
                      <Zap data-icon="inline-start" />
                      立即采集
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => onDelete(s.id)} disabled={scheduleBusy}>
